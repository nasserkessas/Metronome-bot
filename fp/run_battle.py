...
async def async_pick_move(battle):
    ...
    with concurrent.futures.ThreadPoolExecutor() as pool:
        best_move = battle_copy.user.active.moves[0]
        # best_move = await loop.run_in_executor(pool, battle_copy.find_best_move)
        # if not battle_copy.team_preview:
        #     battle_copy.user.update_from_request_json(battle_copy.request_json)
        # best_move = battle_copy.user.active.moves[0]
    return format_decision(battle_copy, best_move.name)
...
async def start_metronome_battle(ps_websocket_client, pokemon_battle_type):
    battle, msg = await start_battle_common(ps_websocket_client, pokemon_battle_type)
    battle.battle_type = constants.RANDOM_BATTLE
    ...
    # apply the messages that were held onto
    process_battle_updates(battle)
    await ps_websocket_client.send_message(battle.battle_tag, ["/move 1 terastal, move 1"])
    return battle

async def start_battle(ps_websocket_client, pokemon_battle_type):
    battle = await start_metronome_battle(ps_websocket_client, pokemon_battle_type)
        ...
        if battle_is_finished(battle.battle_tag, msg):
            ...
            if action_required and not battle.wait:
                if "[Invalid choice]" in msg:
                    oneFainted = True
                if oneFainted:
                    await ps_websocket_client.send_message(battle.battle_tag, ["/move 1"])
                else:
                    await ps_websocket_client.send_message(battle.battle_tag, ["/move 1, move 1"])
